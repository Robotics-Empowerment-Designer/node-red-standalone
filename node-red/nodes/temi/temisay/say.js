module.exports = function (RED) {
    ...
    function say(config) {
        ...
        const finishedTopic = `temi/tts/finished/${node.id}`;
        const { waitIfInterrupted, cleanup } = interruptHandling(node, mqttClient);
        const flow = node.context().flow;
        const global = node.context().global;

        var messageText = config.text || "Standard message, if 'text' is not provided";
        ...
        this.on("input", async function (msg, send, done){
            node.warn("Say node was triggered.");
    ...
    RED.nodes.registerType("temisay", say);
};