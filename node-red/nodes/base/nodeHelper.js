...
traverse(nodesLut, nodeId, startId, startStopLut, joinLut, depth = 100) {
    ...
    if (!nodesLut[nodeId].wires || !Array.isArray(nodesLut[nodeId].wires)) {
        nodesLut[nodeId].wires = []; // Fallback empty array
    }
    ...
    nodesLut[nodeId].wires.forEach(outputs => {
        if (!Array.isArray(outputs)) {
            console.warn(`Outputs of node ${nodeId} are not an array. Skipping.`);
            return;
        }
        ...
        outputs.forEach(wire => {
            ...
            if (!nodesLut[wireString]) {
                console.warn(`Wire ${wireString} not found in nodesLut. Skipping.`);
                return;
            }
